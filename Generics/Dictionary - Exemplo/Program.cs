...
var alunosOrdenados = alunos.OrderBy(x => x.Value.GetNome());
foreach (var item in alunosOrdenados)
{
    Console.WriteLine($"{item.Key} {item.Value.GetNome()} {item.Value.GetNota()}");
}
